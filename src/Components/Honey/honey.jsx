import React from 'react';
import products from '../../assets/honey';
import { Container, Card, CardActions, CardContent, CardMedia, Button, Typography, Box } from '@mui/material';
import Grid from '@mui/material/Grid';   // or '@mui/material/Unstable_Grid2' if you want Grid2

function Honey() {
  return (
    <Box sx={{ my: '5rem' }} id="honey">
      <Typography variant="h5" sx={{ textAlign: 'center' }}>Various Honey Use</Typography>
      <Grid container spacing={3} sx={{ padding: 3, justifyContent: 'center' }}>
        {products.map((product, index) => (
          <Grid item key={index} xs={12} sm={6} md={4}>
            <Card sx={{ maxWidth: 345, textAlign: "center", padding: 2 }}>
              <CardMedia
                component="img"
                height="200"
                image={product.image}
                alt={product.title}
              />
              <CardContent>
                <Typography variant="h6">{product.title}</Typography>
                <Typography variant="body2" color="text.secondary">
                  {product.description}
                </Typography>
                <Typography variant="h6" color="primary" sx={{ marginY: 1 }}>
                  ${product.price.toFixed(2)}
                </Typography>
                <Button variant='contained' color="secondary">
                  Buy Now
                </Button>
              </CardContent>
            </Card>
          </Grid>
        ))}
      </Grid>
    </Box>
  );
}

export default Honey;
